...
<?php else: ?>
	<section>
		<?php foreach($page->images() as $image): ?>
			<img alt="portrait of the photographer" class="db w-100 mw-520 mb-54" width="<?= $image->width() ?>" height="<?= $image->height() ?>" style="aspect-ratio:<?= $image->width() ?>/<?= $image->height() ?>" sizes="(min-width: 520px) 520px, 100vw" srcset="<?= html($image->srcset([600, 800, 1200])) ?>">
		<?php endforeach ?>
	</section>
