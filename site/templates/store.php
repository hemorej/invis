...
<div class="flex items-center flex-wrap gap-52">
	<div class="flex-1 min-w-300">
		<img alt="<?= html($prints->first()->title()) ?> product preview" id="cover" class="db w-100 cover-photo" width="<?= $coverImage->width() ?>" height="<?= $coverImage->height() ?>" style="aspect-ratio:<?= $coverImage->width() ?>/<?= $coverImage->height() ?>" fetchpriority="high" src="<?= html(getPreview($coverImage)) ?>">
	</div>
